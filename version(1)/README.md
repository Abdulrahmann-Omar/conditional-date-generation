...
Best validation CSR after 20 epochs on Modal (A10G, batch 512). The full
table including v2 diffusion models lives in the top-level README.

| Model        | valid | weekday | month | leap  | decade | **all**   |
|--------------|------:|--------:|------:|------:|-------:|----------:|
| GAN          | 0.998 |   0.145 | 0.998 | 0.778 |  0.998 | **0.116** |
| VAE          | 0.980 |   0.144 | 0.980 | 0.755 |  0.980 | **0.111** |
| Transformer  | 1.000 |   0.149 | 1.000 | 1.000 |  1.000 | **0.149** |
| LSTM         | 1.000 |   0.151 | 1.000 | 1.000 |  1.000 | **0.151** |

The weekday number being stuck at ~0.143 is roughly random chance on 7
classes. None of these models actually learns the calendar mapping from
(day, month, year) to weekday. The other three conditions are easy
because they basically come from the inputs through the decoding step.

Transformer and LSTM hit 1.0 on `valid`/`month`/`leap`/`decade` because
they generate the date as a character sequence and learn the format.
GAN and VAE sit slightly below 1.0 there because they emit `(day_idx,
year_digit)` and occasionally produce an out-of-range day (e.g. day 31
in April), which cascades through the validity check.
